...
class Config:
    def __init__(self, fold, config_dict):
        self.fold = fold
        self.config_dict = {}
    ...
    def _load_yamls(self):
        ...
        self.config_dict['data_path'] = self.config_dict['data_path'] + '/fold'+str(self.fold)+'/'
        if not self.config_dict['test_only']:
            self.config_dict['checkpoint_dir'] = self.config_dict['checkpoint_dir'] + str(self.fold)+'/'
            self.config_dict['generated_text_dir'] = self.config_dict['generated_text_dir'] + str(self.fold)+'/'
        else:
            self.config_dict['load_experiment'] = glob.glob(self.config_dict['checkpoint_dir'] + str(self.fold)+'/*')[0]
