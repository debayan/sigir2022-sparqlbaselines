...
class Trainer:
    def __init__(self, config, model, chkpath=None ):
        self.config = config
    ...
    def evaluate(self, eval_data, model_file=None):
        ...
        for source,gold,preds in zip(eval_data.get_reference()[0],eval_data.get_reference()[1],gens):
            ...
            if matchresult:
                qm += 1
            #nonempty = self.kbmatch.returnvalidqueries(source,gold,preds)
            #summary.append(nonempty)
        ...
        #f = open('nonempty_bert_bert_lcq2.json','w')
